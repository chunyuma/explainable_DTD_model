...
def train(epoch, use_gpu, num_epochs, train_loader, train_batch, val_loader, val_batch):
    ...
    for batch_idx, (n_id, adjs) in enumerate(train_loader):
        ...
        random_group_size = len(torch.where(y == 1)[0])
        not_treat_group_size = len(torch.where(y == 2)[0])
        max_size = max(treat_group_size,random_group_size,not_treat_group_size)
        weights = torch.tensor([max_size/treat_group_size,max_size/random_group_size,max_size/not_treat_group_size],dtype=torch.float)
...
#             x_n_id = x[n_id]
            ...
            if use_gpu is True:
                with amp.autocast(enabled=True): # use mixed precision training
                    pred = softmax(model(init_mat, adjs, link, n_id)).detach().cpu().numpy()
            else:
                pred = softmax(model(init_mat, adjs, link, n_id)).detach().cpu().numpy()
    ...
    if calculate_metric is True:
        ...
        macro_f1score = calculate_f1score(probas,labels,'macro')
        ## calculate micro F1 score
        micro_f1score = calculate_f1score(probas,labels,'micro')
        ## calculate one-vs-rest AUC
        ovr_auc_score = met.roc_auc_score(labels, probas, multi_class='ovr')
        ...
        return [acc, macro_f1score, micro_f1score, ovr_auc_score, ovo_auc_score, macro_ap_score, micro_ap_score, labels, probas]
    
    else:
        
        return [None, None, None, None, None, None, None, labels, probas]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_model", type=str, help="Which Model to run: GAT or GraphSage", default="GAT")
    parser.add_argument("--use_concat", type=bool, help="Whether to use concatenation for GraphSage", default=True)
    parser.add_argument("--num_samples", type=str, help="Number of sample in different layers for GraphSage (The length should match 'num_layers')", default="None")
    parser.add_argument("--run_mode", type=int, help="Mode for running model. 1 for normal mode, 2 for crossvalidation", default=1)
    ...
    assert run_model in ['gat', 'graphsage']
    ...
    if args.run_mode == 1:
        ...
        if run_model == 'gat':
            if num_samples is None:
                processdata_path = os.path.join(args.data_path, f'GAT_ProcessedDataset_initemb{init_emb_size}_batch{batch_size}_layer{num_layers}_all_neighbors')
            else:
                processdata_path = os.path.join(args.data_path, f'GAT_ProcessedDataset_initemb{init_emb_size}_batch{batch_size}_layer{num_layers}_neighbors{"+".join([str(num) for num in num_samples])}')
        else:
        ...
        if run_model == 'gat':
            model = GAT(init_emb_size, embedding_size, 3, num_layers=num_layers, dropout_p=dropout_p, num_head = num_head, use_gpu=use_gpu, use_multiple_gpu=use_multiple_gpu)
            if num_samples is None:
                folder_name = f'GAT_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_numhead{num_head}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_all_neighbors'
            else:
                folder_name = f'GAT_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_numhead{num_head}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_neighbors{"+".join([str(num) for num in num_samples])}'
        else:
            model = SAGE(init_emb_size, embedding_size, 3, num_layers=num_layers, dropout_p=dropout_p, use_gpu=True, concat=use_concat, use_multiple_gpu=use_multiple_gpu)
            folder_name = f'GraphSage_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_neighbors{"+".join([str(num) for num in num_samples])}'
        try:
        ...
        for epoch in trange(num_epochs):
            ...
            if val_loss < current_min_val_loss:
                ...
                if run_model == 'gat':
                    if num_samples is None:
                        model_name = f'GAT_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_numhead{num_head}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_all_neighbors.pt'
                    else:
                        model_name = f'GAT_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_numhead{num_head}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_neighbors{"+".join([str(num) for num in num_samples])}.pt'
                else:
                    model_name = f'GraphSage_batchsize{batch_size}_embeddingsize{embedding_size}_layers{num_layers}_lr{lr}_epoch{epoch+1:05d}_val_loss{current_min_val_loss:.5f}_patience{patience}_factor{factor}_neighbors{"+".join([str(num) for num in num_samples])}.pt'
        ...
        train_acc, train_macro_f1score, train_micro_f1score, train_ovr_auc_score, train_ovo_auc_score, train_macro_ap_score, train_micro_ap_score, train_y_true, train_y_probs = evaluate(train_loader, use_gpu, train_batch)
        val_acc, val_macro_f1score, val_micro_f1score, val_ovr_auc_score, val_ovo_auc_score, val_macro_ap_score, val_micro_ap_score, val_y_true, val_y_probs = evaluate(val_loader, use_gpu, val_batch)
        test_acc, test_macro_f1score, test_micro_f1score, test_ovr_auc_score, test_ovo_auc_score, test_macro_ap_score, test_micro_ap_score, test_y_true, test_y_probs = evaluate(test_loader, use_gpu, test_batch)
        ...
        print(f'Final OVR AUC: Train Auc: {train_ovr_auc_score:.5f}, Validation Auc: {val_ovr_auc_score:.5f}, Test Auc: {test_ovr_auc_score:.5f}')
        print(f'Final OVO AUC: Train Auc: {train_ovo_auc_score:.5f}, Validation Auc: {val_ovo_auc_score:.5f}, Test Auc: {test_ovo_auc_score:.5f}')
        print(f'Final Accuracy: Train Accuracy: {train_acc:.5f}, Validation Accuracy: {val_acc:.5f}, Test Accuracy: {test_acc:.5f}')
        print(f'Final Macro F1 score: Train F1score: {train_macro_f1score:.5f}, Validation F1score: {val_macro_f1score:.5f}, Test F1score: {test_macro_f1score:.5f}')
        print(f'Final Micro F1 score: Train F1score: {train_micro_f1score:.5f}, Validation F1score: {val_micro_f1score:.5f}, Test F1score: {test_micro_f1score:.5f}')
        print(f'Final Macro AP score: Train APscore: {train_macro_ap_score:.5f}, Validation APscore: {val_macro_ap_score:.5f}, Test APscore: {test_macro_ap_score:.5f}')
        print(f'Final Micro AP score: Train APscore: {train_micro_ap_score:.5f}, Validation APscore: {val_micro_ap_score:.5f}, Test APscore: {test_micro_ap_score:.5f}')
        ...
        all_evaluation_results['evaluation_ovr_auc_score'] = [train_ovr_auc_score, val_ovr_auc_score, test_ovr_auc_score]
        all_evaluation_results['evaluation_ovo_auc_score'] = [train_ovo_auc_score, val_ovo_auc_score, test_ovo_auc_score]
        all_evaluation_results['evaluation_macro_f1_score'] = [train_macro_f1score, val_macro_f1score, test_macro_f1score]
        all_evaluation_results['evaluation_micro_f1_score'] = [train_micro_f1score, val_micro_f1score, test_micro_f1score]
        all_evaluation_results['evaluation_acc_score'] = [train_acc, val_acc, test_acc]
        all_evaluation_results['evaluation_macro_ap_score'] = [train_macro_ap_score, val_macro_ap_score, test_macro_ap_score]
        all_evaluation_results['evaluation_micro_ap_score'] = [train_micro_ap_score, val_micro_ap_score, test_micro_ap_score]
        all_evaluation_results['evaluation_y_true'] = [train_y_true, val_y_true, test_y_true]
        all_evaluation_results['evaluation_y_probas'] = [train_y_probs, val_y_probs, test_y_probs]
...
#         if run_model == 'gat':
#             if num_samples is None:
#                 processdata_path = os.path.join(args.data_path, f'GAT_crossvalidation_initemb{init_emb_size}_batch{batch_size}_layer{num_layers}_all_neighbors')
#             else:
#                 processdata_path = os.path.join(args.data_path, f'GAT_crossvalidation_initemb{init_emb_size}_batch{batch_size}_layer{num_layers}_neighbors{"+".join([str(num) for num in num_samples])}')
#         else:
#             processdata_path = os.path.join(args.data_path, f'GraphSage_crossvalidation_initemb{init_emb_size}_batch{batch_size}_layer{num_layers}_neighbors{"+".join([str(num) for num in num_samples])}')
#         print('Start pre-processing data', flush=True)
#         dataset = MakeKFoldData(root=processdata_path, run_model=run_model, raw_edges=raw_edges, node_info=node_info, tp_pairs=tp_pairs, tn_pairs=tn_pairs, all_known_tp_pairs=all_known_tp_pairs, K=Kfold, batch_size=batch_size, layers=num_layers, dim=init_emb_size, known_int_emb_dict=known_int_emb_dict, N=mrr_hk_n, num_samples=num_samples)
#         print('Pre-processing data completed', flush=True)
#         del raw_edges, node_info, tp_pairs, tn_pairs, all_known_tp_pairs ## remove the unused varaibles to release memory
#         if args.use_known_embedding:
#             del known_int_emb_dict
...
#             if run_model == 'gat':
#                 model = GAT(init_emb_size, embedding_size, 1, num_layers=num_layers, dropout_p=dropout_p, num_head = num_head, use_gpu=use_gpu, use_multiple_gpu=use_multiple_gpu)
#                 if num_samples is None:
#                     folder_name = f'GAT_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_numhead{num_head}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_all_neighbors'
#                 else:
#                     folder_name = f'GAT_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_numhead{num_head}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_neighbors{"+".join([str(num) for num in num_samples])}'
#             else:
#                 model = SAGE(init_emb_size, embedding_size, 1, num_layers=num_layers, dropout_p=dropout_p, use_gpu=True, concat=use_concat, use_multiple_gpu=use_multiple_gpu)
#                 folder_name = f'GraphSage_batchsize{batch_size}_initemb{init_emb_size}_embeddingsize{embedding_size}_layers{num_layers}_lr{lr}_epoch{num_epochs:05d}_patience{patience}_factor{factor}_neighbors{"+".join([str(num) for num in num_samples])}'
