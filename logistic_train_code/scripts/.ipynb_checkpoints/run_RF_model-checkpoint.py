...
def generate_X_and_y(data_df, curie_to_vec, pair_emb='concatenate'):
    
    if pair_emb == 'concatenate':
        X = np.array([curie_to_vec.loc[curie_to_vec['curie']==data_df.loc[index,'source'],1:].values.tolist()[0] + curie_to_vec.loc[curie_to_vec['curie']==data_df.loc[index,'target'],1:].values.tolist()[0] for index in range(len(data_df))])
        y = np.array(list(data_df['y']))
        
    elif pair_emb == 'hadamard':
        X = np.array([[ a*b for a,b in zip(curie_to_vec.loc[curie_to_vec['curie']==data_df.loc[index,'source'],1:].values.tolist()[0], curie_to_vec.loc[curie_to_vec['curie']==data_df.loc[index,'target'],1:].values.tolist()[0]) ] for index in range(len(data_df))])
        y = np.array(list(data_df['y']))
...
def evaluate(model, X, y_true, calculate_metric=True): 
     ...
     probas = fitModel.predict_proba(X)
...
if __name__ == "__main__":
    ...
    if args.run_mode == 1:
        ...
        with open(args.data_path + 'mode1/train_val_test_random.pkl', 'rb') as infile:
            train_batch, _, test_batch, random_batch = pickle.load(infile)
        ...
        train_X, train_y = generate_X_and_y(train_data, curie_to_vec, pair_emb='concatenate')
        test_X, test_y = generate_X_and_y(test_data, curie_to_vec, pair_emb='concatenate')
        random_X, random_y = generate_X_and_y(random_data, curie_to_vec, pair_emb='concatenate')
        ...
        train_data['prob'] = test_y_probs
        _, _, _, _, _, _, random_y_probs = evaluate(fitModel, random_X, random_y, False)
        ...
        print(f'Final F1 score: Train F1score: {train_f1score:.5f}, Test F1score: {test_f1score:.5f}')
        print(f'Final AP score: Train APscore: {train_ap_score:.5f}, Test APscore: {test_ap_score:.5f}')
        ...
        all_evaluation_results['evaluation_f1_score'] = [train_f1score, test_f1score]
        all_evaluation_results['evaluation_auc_score'] = [train_auc_score, test_auc_score]
        all_evaluation_results['evaluation_ap_score'] = [train_ap_score, test_ap_score]
        all_evaluation_results['evaluation_plot_data'] = [train_plot_data, test_plot_data]
        all_evaluation_results['evaluation_y_true'] = [train_y_true, (test_y_true, random_y_true)]
        all_evaluation_results['evaluation_y_probas'] = [train_y_probs, (test_y_probs, random_y_probs)]
