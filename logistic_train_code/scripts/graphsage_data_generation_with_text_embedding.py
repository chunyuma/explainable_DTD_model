...
parser.add_argument("--emb_file", type=str, help="The initial embedding file")
...
def initialize_node(this):
    ...
    if not has_node_class:
        return [{'test': False, 'id': int(node), 'feature': emb_file[nodes[node]].tolist(), 'label': [1], 'val': (node in valid)}]
    else:
        return [{'test': False, 'id': int(node), 'feature': emb_file[nodes[node]].tolist(), 'label': nodeclass_data_index[nodes[node]][1], 'val': (node in valid)}]
...
if __name__ == "__main__":
    ...
    if args.emb_file == None or not os.path.exists(os.path.realpath(args.emb_file)):
        sys.exit('Error Occurred! Please provide the correct path of your embedding file.')
    else:
    ...
    with open(emb_file_path,'rb') as infile:
